...
import type { AppointmentActionData } from "@/components/admin/AdminAppointmentActions";
import AdminBusinessHourForm from "@/components/admin/AdminBusinessHourForm";
import type { BusinessHourRow } from "@/components/admin/AdminBusinessHourForm";
import AdminSpecialScheduleForm from "@/components/admin/AdminSpecialScheduleForm";
import type { SpecialScheduleRow } from "@/components/admin/AdminSpecialScheduleForm";
import {
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <p className="max-w-3xl text-sm leading-7 text-stone-300 sm:text-base">
              ...
              lugar. La regla de 24h y la validación de slots se aplican automáticamente.
            </p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {statCard({
              ...
              detail: `${data.summary.weekAppointments} en los próximos 7 días`,
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-stone-400">
                        {a.type.replaceAll("_", " ").toLowerCase()} · {a.code}
                      </p>
                    </div>
...
export async function AdminAppointmentsSubroute({
  ...
  switch (subroute) {
    case "agenda":
      return <AgendaSubroute />;
    case "horarios":
      return <HorariosSubroute />;
    case "fechas-especiales":
      return <FechasEspecialesSubroute />;
    default:
      return null;
  }
}
...
async function HorariosSubroute() {
  ...
  return (
    <section className="space-y-6">
      <article className="overflow-hidden rounded-[2rem] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-6 sm:p-8">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-stone-400 sm:text-base">
          Define el horario de atención para cada día de la semana. Las citas solo pueden
...
/* ------------------------------------------------------------------ */
/*  Subroute: Fechas especiales                                        */
/* ------------------------------------------------------------------ */

async function FechasEspecialesSubroute() {
    ...
    <section className="space-y-6">
      <article className="overflow-hidden rounded-[2rem] border border-white/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-6 sm:p-8">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-stone-400 sm:text-base">
          ...
          regular para una fecha específica.
        </p>
      </article>
      ...
      {panel({
        ...
        children: (
          <AdminSpecialScheduleForm
            schedules={schedules as SpecialScheduleRow[]}
          />
        ),
      })}
    </section>
